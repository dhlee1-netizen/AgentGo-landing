...
- **구성 요소**:
  ...
  - 중앙: 앵커 링크 4개 — `왜 AgentGo인가` `6대 모듈` `에디션` `도입 사례`
...
- **콘텐츠 (중앙 정렬)**:
  ...
  [H1] 흩어진 AI를
       하나의 지휘체계로          ← "하나의 지휘체계로" = 그라디언트 텍스트
  [sub] 단 하나의 대화로 모든 업무가 움직입니다.
        보안과 거버넌스를 갖춘 국내 최초
        멀티 에이전트 관리 플랫폼.
...
- **구성**:
  ...
  [모듈 그리드 — 3×2]
  ```
- **모듈 그리드 스타일**: `display: grid`, `grid-template-columns: repeat(3, 1fr)`, `gap: 2px`, border-radius 20px, overflow hidden, border `1px solid glassborder`
- **모듈 6개 콘텐츠**:
  | 태그 | 이름 | 설명 | 키워드 pill |
  |------|------|------|-------------|
...
- **padding**: `72px 24px`
- **구성**: 4개 수치, `grid-template-columns: repeat(4, 1fr)`, 세로 구분선 `1px solid glassborder`
- **수치 4개**:
  | 숫자 | 레이블 |
  |------|--------|
  | 6 | 핵심 관리 모듈 |
  | 3 | 맞춤형 에디션 |
  | 100% | 폐쇄망 지원 (Enterprise) |
  | 1 | 대화로 시작하는 업무 자동화 |
- **수치 스타일**: Inter Black, 52px, `linear-gradient(135deg, #fff, #93C5FD)` 그라디언트 텍스트

---

### 섹션 5 — EDITIONS (에디션 비교)
...
| 접근성 | `prefers-reduced-motion` 미디어쿼리 대응 (애니메이션 비활성화) |
| SEO | `<title>` `<meta description>` `<lang="ko">` 포함 |
...
1. HERO       — 캔버스 배경 + 핵심 메시지 + CTA 2개
2. PROBLEM    — 카드 3개 (Pain point)
3. MODULES    — 3×2 그리드 (6개 모듈)
4. STATS      — 수치 4개 밴드
5. EDITIONS   — 카드 3개 (에디션 비교)
6. PERSONA    — 탭 전환 카드 4개
7. PROCESS    — 3단계 수평 타임라인
8. FAQ        — 아코디언 5개
9. CTA        — 최종 전환 섹션
10. FOOTER    — 브랜드 마무리
```
